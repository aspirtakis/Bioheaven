...
extern double BasketProfit = 50;
 double distance =500;
double timeFrameSRW1 =PERIOD_W1; //RANGE TIMEFRAME
...
//---- sell conditions
   ...
   if(ZZ_1 < ZZ_2 && sells == 0 && upzig < downzag   )  
     {
      res=OrderSend(Symbol(),OP_SELL,Lots,Bid,3,0,0,"AGAS1",MAGICMA,0,Red);
      return;
     }
        if(ZZ_1 < ZZ_2 && sells == 1 && upzig < downzag && Bid >= distancesell  )  
     {
...
//---- buy conditions
   if(ZZ_1 > ZZ_2 && buys == 0  && upzig > downzag )  
     {
      res=OrderSend(Symbol(),OP_BUY,Lots,Ask,3,0,0,"AGAS1",MAGICMA,0,Blue);
      return;
     }
       if(ZZ_1 > ZZ_2 && buys == 1   && upzig > downzag &&Ask <= distancebuy  )  
     {
...
void comments()
   ...
   int  digits = Digits();
     ...
     Comment(StringFormat("\n\nTRADING=%G \DIGITS=%G \BID=%G \ASK=%G \nEQUITY=%G \Equitypercent=%G \R-UP=%G \RDN=%G \RANGE=%G ---DISTANCE=%G \nTAMEIO=%G \nSELLS=%G \BUYS=%G \nSIGNAL=%G\nTREND=%G\nM15=%G\M30=%G\H1=%G\H4=%G\D1=%G\W1=%G\nH1-ADX=%G\nM15-ADX=%G",trading,digits,Bid,Ask,eqt,accequity,rangeUPprc,rangeDNprc,range1,distance,sumbasket,sells,buys,signal,directionma,td5,tdd,tdd4,tdd3,td,wd,adx,adxM5));
   }
