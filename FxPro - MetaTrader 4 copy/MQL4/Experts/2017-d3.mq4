...
extern double Lots  =0.10;
 double BasketProfit = 70;
 double distance =1000;
double timeFrameSRW1 =PERIOD_D1; //RANGE TIMEFRAME
bool closing =false;
...
void zigzag()
    ...
    upzig=iCustom(NULL,PERIOD_H4,"ZigZag", 0, i);
     if(upzig>0) n+=1;
     i++;
   } 
   //  ObjectSet("ZigZagD1up",OBJPROP_PRICE1,upzig);
   //  ObjectSet("ZigZagD1dn",OBJPROP_PRICE1,downzag);
...
//---- go trading only for first tiks of new bar
   ...
   if( sells == 0 && upzig < downzag   )  
     {
      res=OrderSend(Symbol(),OP_SELL,Lots,Bid,3,0,0,"AGAS1",MAGICMA,0,Red);
      return;
     }
        if(sells == 1 && upzig < downzag && Bid >= distancesell  )  
     {
...
void comments()
   ...
   int  digits = Digits();
     ...
     Comment(StringFormat("\n\nTRADING=%G \DIGITS=%G \BID=%G \ASK=%G \nEQUITY=%G \Equitypercent=%G \R-UP=%G \RDN=%G \RANGE=%G ---DISTANCE=%G \nTARGETBASKET=%G \TAMEIO=%G \nSELLS=%G \BUYS=%G \nTREND=%G\nM15=%G\M30=%G\H1=%G\H4=%G\D1=%G\W1=%G\nH1-ADX=%G\nM15-ADX=%G",trading,digits,Bid,Ask,eqt,accequity,rangeUPprc,rangeDNprc,range1,distance,BasketProfit,sumbasket,sells,buys,directionma,td5,tdd,tdd4,tdd3,td,wd,adx,adxM5));
   }
