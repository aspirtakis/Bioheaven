...
void ProfitAdx()
  ...
  if(mode == 0 && tradesides == true){ 
     ...
     return;
       ...
       if( buys < maxbuys && upzig > downzag && buys == 1 && Ask < distancebuy  && alligtrend ==1)
     {
      res=OrderSend(Symbol(),OP_BUY,Lots,Ask,3,0,0,"MODE-0-2B",MAGICMA,0,Blue);
     return;
     }
          if( buys < maxbuys && upzig > downzag && buys >= 2 && Ask < distancebuy  && alligtrend ==1)
     {
     ...
     return;
      ...
      if(sells < maxsells  && upzig < downzag && sells ==1  && Bid > distancesell && alligtrend ==2 ) 
     {
      res=OrderSend(Symbol(),OP_SELL,Lots,Bid,3,0,0,"MODE-0-2S",MAGICMA,0,Red);
     return;
     }
      if(sells < maxsells  && upzig < downzag && sells >=2  &&  Bid > distancesell && alligtrend ==2 ) 
     {
...
void fixdirections()
{
 ...
 void atr()
 {
 // double mak =   iSAR(NULL,PERIOD_D1,0.02,0.2,0);
 // double nik =   iSAR(NULL,PERIOD_H4,0.02,0.2,0);
  //double mak = iATR(NULL,0,12,0);
 //double nik = iATR(NULL,0,20,0);
 alligtrend = 0;
 ...
 double lip=iAlligator(NULL,0,13,8,8,5,5,3,MODE_SMMA,PRICE_MEDIAN,MODE_GATORLIPS,1);
...
void BalanceLots(){
if (balancelots == true){
  if(AccountBalance() >= 200 && AccountBalance() <= 1000)
   {
   Lots = StartLot;
   }
  if(AccountBalance() >= 1000 && AccountBalance() <= 2000)
   {
   Lots = StartLot*2;
   }
  if(AccountBalance() >= 3000 && AccountBalance() <= 4000)
   {
   Lots = StartLot*3;
   }
  if(AccountBalance() >= 4000 && AccountBalance() <= 5000)
   {
   Lots = StartLot*4;
    }
  if(AccountBalance() >= 5000 && AccountBalance() <= 6000)
   {
   Lots = StartLot*5;
   }
  if(AccountBalance() >= 6000 && AccountBalance() <= 7000)
   {
     Lots = StartLot*6;
    }
    ...
    if(AccountBalance() >= 15000 && AccountBalance() <= 20000 )
  {
    Lots = StartLot*15;
      ...
      if(AccountBalance() >= 20000 && AccountBalance() <= 30000 )
  {
    Lots = StartLot*20;
  }
  
   if(AccountBalance() >= 30000 && AccountBalance() <= 50000 )
  {
    Lots = StartLot*25;
  }
